[![Run on Rebyte](https://raw.githubusercontent.com/ReByteAI/run-any-skill-with-single-click/main/badge-v3.svg)](https://app.rebyte.ai/new?prompt=Use%20the%20stock-analysis%20skill.%20List%20the%20catalog%2C%20read%20a%20table%20schema%2C%20and%20run%20a%20small%20LIMIT%20query.)
...
python3 scripts/financial_cli.py catalog

# 2. Read a table's exact columns before querying it
python3 scripts/financial_cli.py schema cn.bars_1m

# 3. Run one read-only SQL statement
python3 scripts/financial_cli.py query "SELECT trade_time, c FROM cn.bars_1m WHERE ts_code = '000001.SZ' ORDER BY trade_time DESC LIMIT 10"
```
